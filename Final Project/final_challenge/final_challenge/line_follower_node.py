...
class line_follower_node(Node):
    ...
    def line_detection_callback(self, msg):
        ...
        img_cropped = img_rotated[int((img_rotated.shape[0])*(7/8)):int((img_rotated.shape[0])), int((img_rotated.shape[1])*2/8):int((img_rotated.shape[1])*7/8)]
...
def main(args = None):
    ...
    line_detection = line_follower_node()
